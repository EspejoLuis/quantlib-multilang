...
TEST_CASE("Number of days in a month works correctly", "[Date]") {
    ...
    SECTION("31-day months") {
        std::vector<Month> monthsWith31Days = {Month::January, Month::March,  Month::May,
                                               Month::July,    Month::August, Month::October,
                                               Month::December};

        /*
        Month m	--> Each loop iteration copies the element into m.
        Month& m --> Each loop iteration gives you a reference to the actual element (can modify
        original). const Month& m --> Reference, but read-only — avoids copies and prevents
        modification. auto m	Compiler --> deduces the type automatically. const auto& m -->
        Compiler deduces type, gives a read-only reference (common for big objects).
        */
    ...
    SECTION("30-day months") {
        std::vector<Month> monthsWith30Days = {Month::April, Month::June, Month::September,
                                               Month::November};
    ...
    SECTION("Invalid month throws") {
        REQUIRE_THROWS_AS(Date::daysInMonth(static_cast<Month>(13), 2019), std::runtime_error);

        REQUIRE_THROWS_WITH(Date::daysInMonth(static_cast<Month>(13), 2019),
...
TEST_CASE("Validate days works correctly - throws error", "[Date]") {
    SECTION("Upper Bound") {
        REQUIRE_THROWS_AS(Date(34, Month::January, 1989), std::out_of_range);

        REQUIRE_THROWS_WITH(Date(34, Month::January, 1989), "Day 34 not between 1 and 31");
    }
    SECTION("Lower Bound") {
        REQUIRE_THROWS_AS(Date(0, Month::January, 1989), std::out_of_range);

        REQUIRE_THROWS_WITH(Date(0, Month::January, 1989), "Day 0 not between 1 and 31");
    }
}

TEST_CASE("Validate year works correctly - throws erros", "[Date]") {
    SECTION("Lower Bound") {
        REQUIRE_THROWS_AS(Date(12, Month::December, 1200), std::out_of_range);

        REQUIRE_THROWS_WITH(Date(12, Month::December, 1200), "Year 1200 not between 1901 and 2199");
    }
    SECTION("Upper Bound") {
        REQUIRE_THROWS_AS(Date(12, Month::December, 3100), std::out_of_range);

        REQUIRE_THROWS_WITH(Date(12, Month::December, 3100), "Year 3100 not between 1901 and 2199");
    }
}
...
TEST_CASE("Normalize method works correctly", "[Date]") {
    ...
    SECTION("Month - Overflow") {
        const auto [startDay, startMonth, startYear, delta, expectedDay, expectedMonth,
                    expectedYear] = GENERATE_COPY(table<int, Month, int, int, int, Month, int>({
        ...
        CAPTURE(startDay, static_cast<int>(startMonth), startYear, delta, expectedDay,
                static_cast<int>(expectedMonth), expectedYear, result.day(), result.month(),
                result.year());
